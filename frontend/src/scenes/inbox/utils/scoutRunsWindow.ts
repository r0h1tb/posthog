...
export function scoutRunOutcomeLabel(run: SignalScoutRunSummary, now: Date): string {
    switch (deriveRunOutcome(run, now)) {
        ...
        case 'running':
            return 'running now'
        case 'stuck':
            return 'running past the deadline and may be stuck'
        case 'queued':
            return 'queued'
        case 'unknown':
            return run.status
...
Then ask me what sort of scout I'd like to make, and offer a few concrete suggestions tailored to what you found (for example specific funnels, error or latency spikes, churn or activation signals, or revenue metrics worth watching). Call out gaps the current fleet doesn't already cover. Once I pick a direction, walk me through authoring the scout end to end.
...
- Whether its scope, thresholds, and schedule look right, with tuning suggestions if needed
