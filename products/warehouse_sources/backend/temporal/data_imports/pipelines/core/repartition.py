...
# Coarser tiers each partition can merge into, coarsest first. A tier only qualifies when it fully
# contains the finer one, so every row's new bucket follows from its old one. ISO weeks straddle month
# boundaries, so rows would move between months in ways the merged key can't express, which leaves a
# week-partitioned table with nowhere coarser to go.
...
def _parse_datetime_partition_key(key: str, partition_format: PartitionFormat) -> datetime | None:
    if partition_format == "week":
        # %G/%V need a weekday to resolve to a date; any day in the week maps to the same coarser bucket.
...
def _simulate_datetime_coarsening(
    ...
    Exact, not estimated: partition keys carry the date they were built from, so the merged layout is
    computed by re-formatting each key. None when any key doesn't parse, because a table carrying the
    unknown-date sentinel or non-date keys must not be coarsened on a guess.
    """
    ...
    for key, size in partition_bytes.items():
        ...
        if parsed is None:
            return None
        merged[parsed.strftime(_DATETIME_KEY_FORMATS[new_format])] += size
    return dict(merged)
...
def select_coarsen_target(
    ...
    if mode == "md5":
        count = schema.partition_count or current_count
        # Halving keeps every candidate a divisor of the current count, which is what makes the
        # simulation exact rather than an assumption about how md5 redistributes rows.
        candidates = []
        candidate = count // 2
        while candidate >= 1 and count % candidate == 0:
            candidates.append(candidate)
            if candidate == 1:
                break
            candidate //= 2
        for new_count in reversed(candidates):
