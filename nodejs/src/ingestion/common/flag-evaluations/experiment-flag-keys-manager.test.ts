...
describe('ExperimentFlagKeysManager', () => {
    ...
    // These drive the loader's response shape directly, so they use a stub router rather
    // than the rows inserted above.
    describe('loader response', () => {
        const managerReturning = (rows: { team_id: number; key: string }[]): ExperimentFlagKeysManager =>
            new ExperimentFlagKeysManager({
                query: jest.fn().mockResolvedValue({ rows }),
            } as unknown as PostgresRouter)

        it('keeps every experiment-backed key for a team, not just the last', async () => {
            const stubbed = managerReturning([
        ...
        it('holds an empty result for the whole null window instead of re-querying every lookup', async () => {
            jest.useFakeTimers({ doNotFake: ['performance'] })
            try {
                const query = jest.fn().mockResolvedValue({ rows: [] })
                const stubbed = new ExperimentFlagKeysManager({ query } as unknown as PostgresRouter)
                // LazyLoader buffers loads for 10ms, so each lookup needs the clock nudged.
                const lookup = async (): Promise<void> => {
                    const pending = stubbed.getExperimentFlagKeys([1])
