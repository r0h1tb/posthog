...
export class ExperimentFlagKeysManager {
    ...
    private async fetchExperimentFlagKeys(teamIds: string[]): Promise<Record<string, Set<string> | null>> {
        ...
        // Every requested team needs an entry, including the ones with no rows.
        // LazyLoader only re-arms cacheUntil for keys the loader returns, so an omitted
        // team holds its null for one refreshNullAgeMs window and then re-queries on
        // every lookup forever after. TeamManager and GroupTypeManager seed the same way.
